...
/**
 * This function searches the pipeline for an approval action that matches the passed customdatamatch. It then returns all details including token and output artifact in case the approval action is in state "InProgress"
 * @param {String} pipelinename
 *  Nane of the AWS CodePipeline
 * @Param {String} customdatamatch
 *  Defines the Approval Action where the UserData matches this string
 ...
 */
exports.findPipelineApprovalActionInProgress = function(pipelinename, customdatamatch, callback) {

    var foundApprovalAction = null;

    console.log("findPipelineApprovalActionInProgress for " + pipelinename + ", " + customdatamatch);
    ...
    codepipeline.getPipeline({name:pipelinename}, function(err, data) {
        ...
        if(foundApprovalAction == null) {
            ...
            callback("No Pipeline Action Found that matches the name and comment!", null);
            return;
        } else {
...
exports.uploadFile = function(outputArtifact, artifactCredentials, filename, content, callback) {
    console.log("uploadeding " + filename + " to " + JSON.stringify(outputArtifact));
